...
export function BeforeAfterSlider({ beforeSrc, afterSrc, height = 420 }: { beforeSrc: string; afterSrc: string; height?: number }) {
  ...
  return (
    <div ref={ref} className="relative overflow-hidden rounded-2xl cursor-ew-resize select-none w-full" style={{ height }} onMouseDown={down} onMouseMove={move} onMouseUp={up} onMouseLeave={up} onTouchStart={down} onTouchMove={move} onTouchEnd={up}>
      <img src={afterSrc} alt="Après" className="absolute inset-0 w-full h-full object-cover" draggable={false} />
      <div className="absolute inset-0 overflow-hidden" style={{ width: `${pos}%` }}>
        <img src={beforeSrc} alt="Avant" className="absolute top-0 left-0 h-full object-cover" style={{ width: w || '100%' }} draggable={false} />
      </div>
      ...
      <div className="absolute top-3 left-3 px-3 py-1 rounded-full text-xs font-bold bg-black/50 text-white backdrop-blur-sm z-20">Avant</div>
      <div className="absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-bold bg-brand-500/90 text-white backdrop-blur-sm z-20">Après</div>
    </div>
  );
}
